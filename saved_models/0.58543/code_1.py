...
train_transforms = transforms.Compose([
    ...
    transforms.RandomRotation(degrees=10),                                                      # was degrees=15
    transforms.RandomAffine(degrees=10, translate=(0.1, 0.1), scale=(0.9, 1.1), shear=5),       # was degrees=15, translate=(0.1, 0.1), scale=(0.8, 1.2), shear=10
    transforms.RandomResizedCrop(size=(224,224), scale=(0.8, 1.0)),                             # was scale=(0.5,1.0)
    ###################################
    ###################################
...
class Classifier(nn.Module):
    def __init__(self, num_classes=7):
        ...
        self.layer_1 = nn.Sequential(
            ...
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2)                 # 80x80 spatial dimension
        )
        self.layer_2 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=3, padding=1),        # 64 channels
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2)                 # 40x40 spatial dimension
        )
        self.layer_3 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=3, padding=1),       # 128 channels
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2)                 # 20x20 spatial dimension
        )
        self.layer_4 = nn.Sequential(
            nn.Conv2d(128, 256, kernel_size=3, padding=1),      # 256 channels
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2)                 # 10x10 spatial dimension
        )
        self.layer_5 = nn.Sequential(
            nn.Conv2d(256, 512, kernel_size=3, padding=1),      # 512 channels
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2)                 # 5x5 spatial dimension
        )
        ...
        self.fc_layer_for_output = nn.Linear(512 * 5 * 5, num_classes)    # 512 channels * 7 spatial height * 7 spatial width
...
optimizer = torch.optim.Adam(model.parameters(), lr=0.00015, weight_decay=1e-5)              # was lr=0.00025, weight_decay=1e-4 
